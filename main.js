...
function bindUI() {
  ...
  on("useHem", "change", () => {
    const hemSide = $("#hemSide");
    if (hemSide) hemSide.disabled = !$("#useHem").checked;
    recalcAll();
  });
  ...
  // Сразу синхронизируем состояние доступности hemSide
  const hemSide = $("#hemSide");
  if (hemSide) hemSide.disabled = !$("#useHem")?.checked;
}
...
function renderWarnings(model) {
  ...
  box.innerHTML = model.warnings.map(w => `• ${w}`).join("<br>");
}

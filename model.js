...
export function updateL(i, v) {
  const minFl = Number($("#minFlange")?.value || 0);
  Ls[i] = Math.max(minFl, Math.max(0, Number(v) || 0));
...
export function addFlange() { Ls.push(Math.max(10, Number($("#minFlange")?.value || 0))); if (Ls.length > 1) As.push(0); emit(); }
...
/**
 * Вычисляет 2D точки профиля из L/∠.
 * Стартовый угол -90° (вверх), чтобы первая полка шла вертикально.
 */
...
export function buildModel() {
  ...
  // Контроль минимальной длины полки
  for (let i = 0; i < Ls.length; i++) {
    const Lval = Math.max(0, Number(Ls[i]) || 0);
    if (minFl > 0 && Lval < minFl) {
      warnings.push(`Полка #${i + 1} меньше минимально допустимой (${Lval} < ${minFl} мм) — будет принята как ${minFl} мм`);
    }
    elements.push({ type: "seg", idx: i, L: Math.max(Lval, minFl) });
  }
  ...
  return { elements, names, segNameMap, blPos, bendNames, width, useHem, side, Ls: [...Ls], As: [...As], warnings };
}
